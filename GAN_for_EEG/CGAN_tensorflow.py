...
import math
#https://blog.csdn.net/tintinetmilou/article/details/81607721关于卷积核的维度问题
#构造可训练参数 tf1.0之前都需要先把要训练的权重定义下来
def make_var(name, shape, trainable = True):
    return tf.get_variable(name, shape, trainable = trainable)#获取一个已经存在的变量或者创建一个新的变量
...
def deconv2d(input_, output_dim, kernel_size, stride, padding = "SAME", name = "deconv2d"):
    ...
    with tf.variable_scope(name):
        ...
        output = tf.nn.conv2d_transpose(input_, kernel, [1, input_height * 2, input_width * 2, output_dim], [1, 2, 2, 1], padding = "SAME")#[filter_height, filter_width, out_channels, in_channels]
        return output
...
def discriminator(image, targets, df_dim=64, reuse=False, name="discriminator"):
	...
	#最后一个卷积模块，输出尺度: 1*32*32*1
        output = conv2d(input_ = h1, output_dim = 1, kernel_size = 4, stride = 1, name='d_h4_conv')
